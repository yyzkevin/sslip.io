...
type Xip struct {
	...
	DelegateLabels              []string                // if set (e.g. ["vm","zone"]), names under <label>.<embedded-ip>... are NS-delegated to the embedded-IP host
...
// delegateLabelTarget returns the delegation target for names under any of the
// configured delegate labels (e.g. with DelegateLabels ["vm"],
// "x.vm.1-2-3-4.example.com." → "1-2-3-4.example.com."), or "" if the
// feature is off, there's no whole-label match, or the remainder to the
// right of the label doesn't embed an IPv4/IPv6 address. When several labels
// match, the leftmost occurrence in the name wins.
func (x *Xip) delegateLabelTarget(fqdn string) string {
	if len(x.DelegateLabels) == 0 {
		return ""
	}
	lower := strings.ToLower(fqdn)
	best := -1
	var target string
	for _, label := range x.DelegateLabels {
		needle := label + "."
		var idx int
		var t string
		if strings.HasPrefix(lower, needle) { // apex: vm.<ip>...
			idx, t = 0, lower[len(needle):]
		} else if i := strings.Index(lower, "."+needle); i >= 0 { // leftmost ".vm."
			idx, t = i+1, lower[i+1+len(needle):]
		} else {
			continue
		}
		if len(NameToA(t, true)) == 0 && len(NameToAAAA(t, true)) == 0 {
			continue // remainder doesn't embed an IP → not a delegation
		}
		if best == -1 || idx < best {
			best, target = idx, t
		}
...
// either an email ("hostmaster@example.com") or the DNS dotted form
// ("hostmaster.example.com"); the first "@" is converted to "." per RFC 1035.
