...
func main() {
	...
	var delegateLabels = flag.String("delegate-labels", "",
		`comma-separated list of DNS labels that trigger stateless subtree delegation: names matching *.<label>.<embedded-ip>.<domain> are NS-delegated to the embedded-IP host. E.g. with -delegate-labels=vm,zone, "x.vm.1-2-3-4.example.com" is delegated to "1-2-3-4.example.com" (the customer's DNS server at 1.2.3.4). Empty = disabled`)
	...
	for _, label := range strings.Split(*delegateLabels, ",") {
		label = strings.ToLower(strings.Trim(strings.TrimSpace(label), "."))
		if label == "" {
			continue
		}
		x.DelegateLabels = append(x.DelegateLabels, label)
	}
	if len(x.DelegateLabels) > 0 {
		log.Printf("Setting delegate labels to %q", x.DelegateLabels)
	}
	if *soaEmail != "" {
