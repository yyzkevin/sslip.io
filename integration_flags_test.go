...
var _ = Describe("flags", func() {
	...
	When("-whitelistURL is set", func() {
		BeforeEach(func() {
			flags = []string{"-whitelistURL=file://etc/whitelist-test.txt"}
		})
		It("resolves an A record whose IP is inside a whitelisted prefix", func() {
			digArgs := "@localhost 203-0-113-5.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr aa rd; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0`))
			Eventually(digSession).Should(Say(`203.0.113.5\n`))
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeA 203-0-113-5\.example\.com\. \? 203\.0\.113\.5`))
		})
		It("does NOT resolve an A record whose IP is outside every whitelisted prefix", func() {
			digArgs := "@localhost 8-8-8-8.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`\? nil, SOA 8-8-8-8\.example\.com\. briancunnie\.gmail\.com\.`))
		})
		It("resolves an AAAA record whose IP is inside a whitelisted prefix", func() {
			digArgs := "@localhost 2001-db8-abcd--1.example.com aaaa -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr aa rd; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0`))
			Eventually(digSession).Should(Say(`2001:db8:abcd::1\n`))
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeAAAA 2001-db8-abcd--1\.example\.com\. \? 2001:db8:abcd::1`))
		})
		It("does NOT resolve an AAAA record whose IP is outside every whitelisted prefix", func() {
			digArgs := "@localhost 2001-db8-ffff--1.example.com aaaa -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`\? nil, SOA 2001-db8-ffff--1\.example\.com\. briancunnie\.gmail\.com\.`))
		})
	})
	When("-hosts-file is set", func() {
		BeforeEach(func() {
			flags = []string{"-hosts-file=etc/hosts-test.txt"}
		})
		It("resolves the static A record from the file", func() {
			digArgs := "@localhost ns1.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr aa rd; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0`))
			Eventually(digSession).Should(Say(`1.2.3.4\n`))
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`Adding record "ns1.example.com.=1.2.3.4"`))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeA ns1\.example\.com\. \? 1\.2\.3\.4`))
		})
		It("resolves the static AAAA record from the file", func() {
			digArgs := "@localhost ns1.example.com aaaa -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr aa rd; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0`))
			Eventually(digSession).Should(Say(`2001:db8::1\n`))
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeAAAA ns1\.example\.com\. \? 2001:db8::1`))
		})
	})
	When("-whitelistURL and -hosts-file are both set", func() {
		BeforeEach(func() {
			flags = []string{"-whitelistURL=file://etc/whitelist-test.txt", "-hosts-file=etc/hosts-test.txt"}
		})
		It("still resolves a static record whose IP is outside the whitelist (operator records are exempt)", func() {
			digArgs := "@localhost ns1.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
		...
		It("still rejects an embedded IP outside the whitelist", func() {
			digArgs := "@localhost 8-8-8-8.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
	...
	When("-delegate-labels is set", func() {
		BeforeEach(func() {
			flags = []string{"-delegate-labels=zone,vm", "-whitelistURL=file://etc/whitelist-test.txt"}
		})
		It("NS-delegates names under the label to the embedded-IP host, with glue", func() {
			digArgs := "@localhost anything.zone.203-0-113-5.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr rd; QUERY: 1, ANSWER: 0, AUTHORITY: 1, ADDITIONAL: 1`))
			Eventually(digSession).Should(Say(`;; AUTHORITY SECTION:`))
			Eventually(digSession).Should(Say(`NS\s+203-0-113-5\.example\.com\.\n`))
			Eventually(digSession).Should(Say(`;; ADDITIONAL SECTION:`))
			Eventually(digSession).Should(Say(`203\.0\.113\.5\n`))
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`anything\.zone\.203-0-113-5\.example\.com\. \? nil, NS 203-0-113-5\.example\.com\.`))
		})
		It("delegates every configured label, not just the first", func() {
			digArgs := "@localhost anything.vm.203-0-113-5.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`flags: qr rd; QUERY: 1, ANSWER: 0, AUTHORITY: 1, ADDITIONAL: 1`))
		...
		It("delegates ACME challenges under the label in one hop", func() {
			digArgs := "@localhost _acme-challenge.x.zone.203-0-113-5.example.com txt -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`;; AUTHORITY SECTION:`))
		...
		It("does NOT delegate when the embedded IP is outside the whitelist", func() {
			digArgs := "@localhost anything.zone.8-8-8-8.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`\? nil, SOA anything\.zone\.8-8-8-8\.example\.com\. briancunnie\.gmail\.com\.`))
		})
		It("matches whole labels only: myzone still resolves to the embedded IP", func() {
			digArgs := "@localhost anything.myzone.203-0-113-5.example.com a -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession).Should(Say(`;; ANSWER SECTION:`))
	...
	When("-soa-email and -soa-mname are set", func() {
		BeforeEach(func() {
			flags = []string{"-soa-email=hostmaster@example.com", "-soa-mname=ns1.example.com"}
		})
		It("returns the configured MNAME and contact in the SOA record", func() {
			digArgs := "@localhost nonexistent.example.com soa -p " + strconv.Itoa(port)
			digCmd := exec.Command("dig", strings.Split(digArgs, " ")...)
			digSession, err := Start(digCmd, GinkgoWriter, GinkgoWriter)
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(digSession.Out.Contents())).Should(MatchRegexp(`ns1\.example\.com\. hostmaster\.example\.com\.`))
		})
	})
})
