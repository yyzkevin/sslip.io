...
var _ = Describe("flags", func() {
	...
	When("-delegate-label is set", func() {
		BeforeEach(func() {
			flags = []string{"-delegate-label=zone", "-whitelistURL=file://etc/whitelist-test.txt"}
		})
	...
	When("-soa-email and -soa-mname are set", func() {
		BeforeEach(func() {
			flags = []string{"-soa-email=support@inverdigm.ca", "-soa-mname=ns1.leafcloud.ca"}
		})
		It("returns the configured MNAME and contact in the SOA record", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Eventually(digSession, 1).Should(Exit(0))
			Eventually(string(digSession.Out.Contents())).Should(MatchRegexp(`ns1\.leafcloud\.ca\. support\.inverdigm\.ca\.`))
		})
	})
})
